...
過去の制作物（Projects）と現在進行形の開発記録（DevLog）を管理・公開することを目的とする。コンテンツの管理は外部サービス（Notion, Zenn）で行い、静的サイトとしてビルドして公開するモダンな構成を採用する。

## 2. 技術スタック
AIエージェントは以下の技術スタックを前提にコードを生成すること。
* **Framework:** Next.js (App Router) または Astro （静的サイト生成(SSG)を前提とする）
* **Styling:** Tailwind CSS
* **Content Source:** * Projects: Notion API
...
## 🤖 AI Agentへの初期プロンプト（Vibe Coding Instructions）
AIエージェントは、このプロジェクトを初期化する際、以下のステップで実行してください。
1. Next.js (App Router) と Tailwind CSS の初期セットアップを行う。
2. ヘッダー（ハンバーガーメニュー含む）、フッター、カードコンポーネントのUIベースを作成する。
3. ルーティング (`/`, `/projects`, `/devlog`, `/about`) のスケルトンを作成する。
4. Zenn風のマークダウンレンダリングを行うためのプラグイン群（`remark-gfm`, `rehype-highlight` 等）を導入し、詳細ページのコンポーネントを実装する。
